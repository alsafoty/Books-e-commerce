<template>
  <div class="category-products-page">
    ...
    <!-- Header Section -->
    <div class="category-products-header bg-light py-3 py-md-4" dir="rtl">
      <div
              ...
              <ol class="breadcrumb gap-1 mb-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">
                  <span class="d-none d-md-inline">{{
                    categoryInfo ? categoryInfo.name : "منتجات الفئة"
                  }}</span>
                  <span class="d-md-none">{{
                    categoryInfo
                      ? categoryInfo.name.substring(0, 15) + "..."
                      : "منتجات"
                  }}</span>
                </li>
              </ol>
            ...
            >
              <div class="col-12 col-lg-4">
                <h1 class="h4 h2-md fw-bold text-primary mb-0">
                  ...
                  <span class="d-none d-md-inline">{{
                    categoryInfo ? categoryInfo.name : "المنتجات"
                  }}</span>
          ...
          <div class="col-md-6">
            <div class="empty-state-card p-5 rounded-4 shadow-sm bg-light">
              ...
              <p class="text-muted mb-4">
                لا توجد منتجات متاحة في هذه الفئة في الوقت الحالي.
              </p>
...
const fetchCategoryProducts = async () => {
  try {
    ...
    const categoryResponse = await axios.get(
      `${process.env.VUE_APP_API_BASE_URL}/category/${categoryId.value}`,
      {
        headers: {
    ...
    document.title = ` متجر الكتب | ${categoryInfo.value?.name || "المنتجات"} `;

    // Fetch products for this category
    const productsResponse = await axios.get(
      `${process.env.VUE_APP_API_BASE_URL}/product/browse?categoryId=${
        categoryId.value
      }&limit=${parseInt(productsPerPage.value)}&sortBy=${sortBy.value}&order=${
        sortOrder.value
...
const getProductsCount = async () => {
  try {
    axios
      .get(
        `${process.env.VUE_APP_API_BASE_URL}/product/browse?searchTerm=${searchQuery.value}&categoryId=${categoryId.value}`,
        {
          headers: {
            "Content-Type": "application/json",
          },
...
onMounted(() => {
  if (categoryId.value) {
    ...
    showMessage("معرف الفئة مطلوب", "danger");
    router.push("/categories");
  }
});
</script>
